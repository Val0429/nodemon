...
function run(options) {
  ...
  /// Val: here spawn new process, to handle children script
  /// Example, cmd: `npm run build`
  var path = require("path");
  var fs = require("fs");
  function findModuleEntryPoint(module) {
    const NODE_MODULE_PATH = "./node_modules";
    const SPECIAL_MODULE_PATH = "./node_modules/@valuis0429";
    var modulePath = path.resolve((process.pkg||{}).defaultEntrypath, SPECIAL_MODULE_PATH, module);
    if (!fs.existsSync(modulePath)) {
      modulePath = path.resolve((process.pkg||{}).defaultEntrypath, NODE_MODULE_PATH, module);
      if (!fs.existsSync(modulePath)) {
        throw `Val: Module <${module}> not exists`;
      }
    }
    var packagePath = path.resolve(modulePath, "package.json");
    if (!fs.existsSync(packagePath)) throw `Val: Module <${module}> package.json file not exists`;

    var pkg = JSON.parse(fs.readFileSync(packagePath).toString());
    var config = pkg.bin || pkg.main;
    if (typeof config === "object") config = config[module];
    
    return path.resolve(modulePath, config);
  }
  ...
  if (shouldFork) {
    ...
    /// Val: edited
    if (process.pkg) {
      var sh = process.argv[0];
      var cmds = args.split(" ");
      try {
        cmds[0] = `"${findModuleEntryPoint(cmds[0])}"`;
      } catch(e) {
        console.error(e);
      }
      spawnArgs = [`"${sh}"`, cmds, { ...spawnOptions, shell: true }];
    }
  ...
  child.on('exit', function (code, signal) {
    ...
    if (signal === config.signal || code === 0) {
      ...
      } else if (options.restartcrash) {
        bus.emit('crash');
        child = null;
        setTimeout(() => {
          utils.log.fail('app crashed - auto restart due to settings...');
          bus.emit('restart');
        }, 100);
