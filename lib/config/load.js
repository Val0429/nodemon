...
function load(settings, options, config, callback) {
  ...
  loadFile(options, config, utils.home, function (options) {
    ...
    /// Val: modified, packaged path
    loadFile(options, config, (process.pkg||{}).defaultEntrypath || process.cwd(), function (options) {
    ...
    if (err) {
      if (err.code === 'ENOENT') {
        if (!options.configFile && dir !== utils.home) {
          ...
          return loadPackageJSON(config, callback, dir);
        }
      }
      return callback({});
    }
...
function loadPackageJSON(config, ready, dir) {
  if (!ready) {
    ready = () => { };
  }

  /// Val: modified, look for dir
  dir = dir || process.cwd();
